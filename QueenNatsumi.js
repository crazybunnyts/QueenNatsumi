...
// ════════════════════STARTED SQL🍁🍁
const NatsumiDB = config.DATABASE.define('QueenNatsumi', {
    info: {
...
// ════════════════════END PLUGGINS SUCCESS🍁🍁🍁
        ...
        console.log(
         ...
         if (config.LANG == 'EN') {
             await conn.sendMessage(conn.user.jid, fs.readFileSync("./media/Natsumi.jpg"), MessageType.image, { caption: `🪀💃 MERRY CHRISTMAS 🏆!!  ${conn.user.name}! \n\n*❒ Welcome To QUEEN-NATSUMI :│🍁*\n\n\n Your Bot Working  As ${config.WORKTYPE} ❒.\n\n*❒│QUEEN-NATSUMI WORKING Your Account*\n\n*🍁▷ Use the 🚀.NATSUMI command to get bot menu...*\n\n\n*❒ QUEEN-NATSUMI is a powerfull WhatsApp robot developed by DINUKA.*\n\n*🚀 This is your LOG number. Avoid using the command here.\n\n❒ .new Command use for new items*\n\n`});
             
         } else if (config.LANG == 'SI') {
             await conn.sendMessage(conn.user.jid, fs.readFileSync("./media/Natsumi.jpg"), MessageType.image, { caption: `🪀💃 සුබ නත්තලක් 🏆!!  ${conn.user.name}! \n\n*❒ සාදරයෙන් QUEEN-NATSUMI වෙත පිලිගන්නවා :│🍁*\n\n\n ඔබේ Bot ${config.WORKTYPE} ලෙස ක්‍රියාකරයි.\n\n*❒│QUEEN-NATSUMI ඔබගේ ගිණුමේ දැන් සක්‍රියයි*\n\n*🍁 QUEEN-NATSUMI bot සම්පූර්න මෙනුව ලබා ගැනීමට ❒.NATSUMI විධානය භාවිතා කරන්න...*\n\n\n*❒ QUEEN-NATSUMI යනූ සීඝ්‍රයෙන් වර්ධනය වන Whatsapp රොබෝවෙකි..QUEEN-NATSUMI වෙත ලැබෙන නව අංග හා යතාවත්කාලින කිරිම් ලබා ගැනීමට🍁 .new විධානය භාවිතා කරන්න..*\n\n*😈 මෙය ඔබගේ LOG අංකයයි.මෙහි විධාන භාවිතයෙන් වළකින්න.*\n\n`});
             
         } else {
             await conn.sendMessage(conn.user.jid, fs.readFileSync("./media/Natsumi.jpg"), MessageType.image, { caption: `🪀💃 MERRY CHRISTMAS 🎟️!!  ${conn.user.name}! \\nn*❒ Welcome To QUEEN-NATSUMI :│🍁*\n\n\n Your Bot Working  As ${config.WORKTYPE} ❒.\n\n*❒ │QUEEN-NATSUMI WORKING Your Account*\n\n*🍁▷ Use the 🚀.NATSUMI command to get bot menu...*\n\n\n*❒ QUEEN-NATSUMI is a powerfull WhatsApp robot developed by DINUKA.*\n\n*🚀 This is your LOG number. Avoid using the command here.\n\n❒ .new Command use for new items*\n\n`});
        }
     });
    ...
    setInterval(async () => { 
        if (config.AUTOBIO == 'true') {
            if (conn.user.jid.startsWith('90')) { 
                ...
                const biography = '📅 ' + utch + '\n⌚ ' + ov_time + '\n\nPOWERD BY 🪀 QUEEN-NATSUMI BOT 💃⚙️️'
...
*/
                        try {
                            ...
                            if (config.LANG == 'EN') {
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./media/ERR.jpg"), MessageType.image, { caption: '*🎯💃 QUEEN-NATSUMI BOT 🪀*  WORKING AS '+config.WORKTYPE+'!!\n\nYOUR NAME ▷${conn.user.name}! /n/n ⦁_This is your LOG number Dont Try Command here_\n▷Also You Can join Our Support group More Help.\n_❒Support 01▷ https://chat.whatsapp.com/GT5V8RakkftB7DAKWMeQML\n\n*Error:* ```' + error + '```\n\n' });
                                
                            } else if (config.LANG == 'SI') {
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./media/ERR.jpg"), MessageType.image, { caption: '*🏅💃 QUEEN-NATSUMI BOT 🪀*  '+config.WORKTYPE+' ලෙස ක්‍රියා කරයි!!\n\nඔබේ නම ▷${conn.user.name}! /n/n ⦁_මෙය ඔබගේ LOG අංකයයි මෙහි විධන භාවිතයෙන් වළකින්න_\n▷ඔබට යම් ගැටලුවක් ඇත්නම් අපගේ සහය සමූහට ලිවිය හැක.\nf_❒Support 01▷ https://chat.whatsapp.com/GT5V8RakkftB7DAKWMeQML\n\n*දෝෂය:* ```' + error + '```\n\n' });
                                
                            } else {
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./media/ERR.jpg"), MessageType.image, { caption: '*💃🎟️ QUEEN-NATSUMI BOT 🪀*  WORKING AS '+config.WORKTYPE+'!!\n\n YOUR NAME ▷ ${conn.user.name}! /n/n ⦁_This is your LOG number Dont Try Command here_\n▷Also You Can join Our Support group More Help.\n_❒Support 01▷ https://chat.whatsapp.com/GT5V8RakkftB7DAKWMeQML\n\n*Error:* ```' + error + '```\n\n' });
                            }
                        }
                    }
                }
